document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('click', function (e) {
    ...
    var navbarEl = document.querySelector('.navbar, header, nav');
    var offset = navbarEl ? navbarEl.offsetHeight : 0;
    var elementTop = destination.getBoundingClientRect().top + window.pageYOffset;
  ...
  if (contactForm) {
    ...
    function clearError(input) {
      ...
      if (next && next.classList && next.classList.contains('form-error-message')) {
        next.parentNode.removeChild(next);
      }
    }

    function showFormMessage(form, type, text) {
      var existing = form.querySelector('.form-status-message');
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        var firstError = contactForm.querySelector('.input-error');
        if (firstError) {
          var navbarHeight = navbar ? navbar.offsetHeight : 0;
          var top = firstError.getBoundingClientRect().top + window.pageYOffset - navbarHeight - 20;
          window.scrollTo({ top: top, behavior: 'smooth' });
          firstError.focus();
        }
        showFormMessage(contactForm, 'error', 'Please correct the errors above and try again.');
        return;
      }
      ...
      fetch(contactForm.action || window.location.href, {
        ...
        .then(function (response) {
          if (response.ok) {
            showFormMessage(contactForm, 'success', 'Thank you! Your message has been sent successfully.');
            contactForm.reset();
          } else {
        ...
        .catch(function () {
          showFormMessage(contactForm, 'error', 'Something went wrong. Please try again later.');
        })
        .finally(function () {
          if (submitBtn) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '5ae60aaf-9a00-49a2-8e02-a62ca099f9d0',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '5ae60aaf-9a00-49a2-8e02-a62ca099f9d0';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '5ae60aaf-9a00-49a2-8e02-a62ca099f9d0';
